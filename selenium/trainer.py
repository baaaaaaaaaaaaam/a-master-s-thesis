    ...
    for line in data.split('\n'):
        if line.strip():  # 빈 줄은 무시
            ...
            try:
                ...
                age = fields[3]
                debut_date = fields[4]
                total_appearances = int(fields[5])
                total_first_place = int(fields[6])
                total_second_place = int(fields[7])
                total_third_place = int(fields[8])
                yearly_appearances = int(fields[9])
                yearly_first_place = int(fields[10])
                yearly_second_place = int(fields[11])
                yearly_third_place = int(fields[12])
                # 데이터 삽입 쿼리
                insert_query = """
                INSERT INTO trainer (
                    round, name, birth_date, age, debut_date, 
                ...
                mycursor.execute(insert_query, (
                    num, name, birth_date, age, debut_date, 
                    total_appearances,total_first_place, total_second_place, total_third_place,
                    yearly_appearances, yearly_first_place, yearly_second_place, yearly_third_place,
...
def runSelenium(day):
    ...
    time.sleep(2)
    url = f'https://race.kra.co.kr/dbdata/fileDownLoad.do?fn=internet/seoul/trainer/{day}sdb3.txt&meet=1'
    driver.get(url)
